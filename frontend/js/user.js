...
async function loadUserProfile() {
    try {
    const response = await fetch('https://localhost:7240/api/users/infome', { credentials: 'include' });
    if (!response.ok) {
...
async function updateProfile() {
    ...
    const response = await fetch(`https://localhost:7240/api/users/infome/name`, {
    method: 'PUT',
    credentials: 'include',
...
async function logout() {
    const response = await fetch('https://localhost:7240/api/Users/logout', {
    method: 'POST',
    credentials: 'include'
    });
...
async function loadAllUsers() {
    try {
    const response = await fetch('https://localhost:7240/api/users', { credentials: 'include' });
    if (!response.ok) {
...
async function loadAllProducts() {
    try {
    const response = await fetch('https://localhost:7240/api/products', { credentials: 'include' });
    if (!response.ok) {
...
async function saveProductChanges() {
    ...
    const response = await fetch(`https://localhost:7240/api/products/${editingProductId}`, {
    method: 'PUT',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
