...
async function loadUserProfile() {
    try {
    const response = await fetch(`${API_BASE_URL}/api/users/infome`, { credentials: 'include' });
    if (!response.ok) {
...
async function updateProfile() {
    ...
    const response = await fetch(`${API_BASE_URL}/api/users/infome/name`, {
    method: 'PUT',
    credentials: 'include',
...
async function changePassword() {
    ...
    // return;
    // }
    const response = await fetch(`${API_BASE_URL}/api/users/infome/password`, {
    method: 'PATCH',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
